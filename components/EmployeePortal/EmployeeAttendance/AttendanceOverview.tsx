...
import { 
  Clock, Calendar, RotateCcw, Gift, ChevronRight, TrendingUp, 
  Timer, CheckCircle2, AlertCircle, BarChart3, Shield, 
  Zap, Users, Briefcase, MapPin, Sun, Moon 
} from 'lucide-react';
...
const PRIORITY_LOGIC = [
  { priority: 1, label: 'Approved Leave', color: '#3b82f6', description: 'Highest priority — overrides all', icon: Calendar },
  { priority: 2, label: 'Approved Regularization', color: '#10b981', description: 'Corrects raw attendance data', icon: RotateCcw },
  { priority: 3, label: 'System Attendance', color: '#2D7A4F', description: 'Raw check-in/out logs', icon: Clock },
  { priority: 4, label: 'Absence (Default)', color: '#ef4444', description: 'When no data exists', icon: AlertCircle },
];

const QUICK_STATS = [
  { label: 'Attendance Rate', value: '92%', change: '+12%', color: '#2D7A4F', bg: '#e8f5ee' },
  { label: 'Days Present', value: '18', change: '+2', color: '#3b82f6', bg: '#eff6ff' },
  { label: 'Leaves Taken', value: '2', change: '-1', color: '#f59e0b', bg: '#fffbeb' },
  { label: 'Comp Off Balance', value: '2', change: 'Available', color: '#8b5cf6', bg: '#f5f3ff' },
];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="w-full px-4 lg:px-8 py-6">
        {/* Header Section */}
        <div className="mb-6">
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Attendance Module</h1>
          <p className="text-sm text-gray-500 mt-1">Manage your attendance, leaves, and regularizations</p>
        </div>
          ...
          <div className="relative z-10">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 bg-[#2D7A4F]/30 border border-[#2D7A4F]/40 rounded-full px-4 py-2">
                ...
                <span className="text-[#4a9e6e] text-sm font-semibold">Checked in · 09:12 AM</span>
              </div>
              <div className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-full px-4 py-2">
                ...
                <span className="text-white/60 text-sm font-semibold">5h 43m worked today</span>
              </div>
              <div className="flex items-center gap-2 bg-amber-500/20 border border-amber-500/20 rounded-full px-4 py-2">
                ...
                <span className="text-amber-300 text-sm font-semibold">1 pending approval</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: stat.bg }}>
                  <TrendingUp size={18} style={{ color: stat.color }} />
                </div>
                <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ backgroundColor: stat.bg, color: stat.color }}>
                  {stat.change}
                </span>
              </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-bold text-gray-800">Attendance Tools</h2>
              <p className="text-xs text-gray-500 mt-0.5">Everything you need to manage your attendance</p>
            </div>
            <Link href="/employee/attendance/all" className="text-sm font-semibold text-[#2D7A4F] hover:underline">
              View All →
            </Link>
          </div>
            ...
            {ATTENDANCE_MODULES.map((mod) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 mt-2">
                  <span className="text-sm font-semibold" style={{ color: mod.color }}>Access Module</span>
                  <ChevronRight 
                    size={14} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h3 className="text-base font-bold text-gray-800">Final Attendance Resolution</h3>
                <p className="text-xs text-gray-500">Priority-based attendance calculation</p>
              </div>
            ...
            <p className="text-sm text-gray-600 mb-5">
              All inputs converge into one final attendance truth. Higher priority always wins.
            </p>
            
            <div className="space-y-3">
              {PRIORITY_LOGIC.map((item) => (
                <div key={item.priority} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                  <div
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/employee/attendance/history" className="text-xs font-semibold text-[#2D7A4F] hover:underline">
                  View All →
                </Link>
              </div>
              ...
              <div className="space-y-3">
                {[
                  { action: 'Checked in', time: 'Today at 09:12 AM', icon: Clock, color: '#2D7A4F' },
                  { action: 'Leave Applied', time: 'Mar 20, 2026 · Casual Leave', icon: Calendar, color: '#3b82f6' },
                  { action: 'Regularization Approved', time: 'Mar 19, 2026 · Corrected entry', icon: RotateCcw, color: '#10b981' },
                ].map((item, idx) => (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Regularize missed punches within 7 days to maintain accurate attendance records.
                  </p>
                  <Link href="/employee/attendance/regularize" className="inline-flex items-center gap-1 mt-3 text-sm font-semibold text-blue-600 hover:underline">
                    Regularize now
