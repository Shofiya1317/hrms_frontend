...
// ── Data ──────────────────────────────────────────────────────────
const LEAVE_TYPES = [
  { type: 'Annual Leave',      days: 18,  carry: true,  encash: true,  icon: Palmtree,   color: 'text-emerald-500', bg: 'bg-emerald-50' },
...
const LEAVE_REQUESTS = [
  { id: 1, name: 'Rohit Gupta',     type: 'Annual Leave',  from: '22 Mar', to: '24 Mar', days: 3, status: 'pending',  reason: 'Family vacation',    avatar: 'RG', color: 'bg-emerald-500' },
...
const EMPLOYEES = [
  { name: 'Rahul Sharma',   annual: [13, 18], sick: [10, 12], casual: [5, 6],  avatar: 'RS', color: 'bg-emerald-500' },
...
const statusConfig = {
  ...
  approved: { label: 'Approved', cls: 'bg-emerald-50 text-emerald-700 border border-emerald-200', dot: 'bg-emerald-400', icon: CheckCircle2 },
...
function SummaryStrip() {
  ...
  const cards = [
    ...
    { label: 'Approved',       value: approved, icon: CheckCircle2,  bg: 'bg-emerald-50', text: 'text-emerald-600' },
      ...
      {cards.map(c => (
        <div key={c.label} className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-slate-400 font-medium mt-0.5 truncate">{c.label}</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3 py-2 rounded-xl transition-colors">
      ...
      <div className="divide-y divide-slate-50">
        {LEAVE_REQUESTS.map(req => {
          ...
          return (
            <div key={req.id} className="group px-5 py-4 hover:bg-slate-50/60 transition-colors">
              <div className="flex items-start gap-3 sm:gap-4">
                {/* Avatar */}
                <div className={`w-10 h-10 rounded-xl ${req.color} flex items-center justify-center flex-shrink-0 text-white text-xs font-bold shadow-sm`}>
                  {req.avatar}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {/* Status badge */}
                    <span className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${sc.cls}`}>
                ...
                {/* Actions — right side on sm+, bottom on mobile */}
                {req.status === 'pending' && (
                  <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                    <button className="flex items-center gap-1.5 text-xs font-semibold text-white bg-emerald-500 hover:bg-emerald-600 active:scale-95 px-3 py-2 rounded-xl transition-all">
                      <CheckCircle2 size={13} /> Approve
                    </button>
                    <button className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 active:scale-95 px-3 py-2 rounded-xl transition-all">
              ...
              {/* Mobile action row */}
              {req.status === 'pending' && (
                <div className="flex sm:hidden items-center gap-2 mt-3 ml-13">
                  <button className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-white bg-emerald-500 hover:bg-emerald-600 py-2.5 rounded-xl transition-all active:scale-95">
                    <CheckCircle2 size={13} /> Approve
                  </button>
                  <button className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 py-2.5 rounded-xl transition-all active:scale-95">
                    <XCircle size={13} /> Reject
                  </button>
                </div>
              )}
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
        ...
        <button className="self-start sm:self-auto flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 active:scale-95 rounded-xl transition-all shadow-sm">
          <Plus size={13} /> Add Type
        </button>
      </div>

      {/* Card grid on mobile, table on sm+ */}
      <div className="sm:hidden divide-y divide-slate-50">
        {LEAVE_TYPES.map(lt => (
          <div key={lt.type} className="px-5 py-4 flex items-center gap-4">
            <div className={`w-10 h-10 rounded-xl ${lt.bg} flex items-center justify-center flex-shrink-0`}>
            ...
            <div className="flex flex-col items-end gap-1">
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${lt.carry ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                {lt.carry ? 'Carry ✓' : 'No carry'}
              </span>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${lt.encash ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
                {lt.encash ? 'Encash ✓' : 'No encash'}
              </span>
            </div>
          </div>
        ))}
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full min-w-[580px]">
          ...
          <tbody className="divide-y divide-slate-50">
            {LEAVE_TYPES.map(lt => (
              <tr key={lt.type} className="group hover:bg-slate-50/60 transition-colors">
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg ${lt.bg} flex items-center justify-center flex-shrink-0`}>
                ...
                <td className="px-5 py-3.5">
                  ...
                  {lt.days === 0 && <span className="text-xs text-slate-400 ml-1">As earned</span>}
                </td>
                <td className="px-5 py-3.5">
                  <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${lt.carry ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>
                    {lt.carry && <CheckCircle2 size={11} />} {lt.carry ? 'Yes' : 'No'}
                  </span>
                </td>
                <td className="px-5 py-3.5">
                  <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${lt.encash ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-400'}`}>
                    {lt.encash && <CheckCircle2 size={11} />} {lt.encash ? 'Yes' : 'No'}
                  </span>
                </td>
                <td className="px-5 py-3.5">
                  <button className="opacity-0 group-hover:opacity-100 text-[11px] font-medium text-slate-400 hover:text-slate-700 flex items-center gap-1 transition-all">
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
        ...
        <button className="self-start sm:self-auto flex items-center gap-2 text-xs font-medium text-slate-500 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3 py-2 rounded-xl transition-colors">
        ...
        {EMPLOYEES.map(emp => (
          <div key={emp.name} className="px-5 py-4">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-9 h-9 rounded-xl ${emp.color} text-white text-xs font-bold flex items-center justify-center flex-shrink-0 shadow-sm`}>
                {emp.avatar}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-900 truncate">{emp.name}</p>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {emp.annual[0] + emp.sick[0] + emp.casual[0]} days used of {emp.annual[1] + emp.sick[1] + emp.casual[1]} total
                </p>
              </div>
              <button className="hidden sm:flex items-center gap-1 text-[11px] font-medium text-indigo-600 hover:text-indigo-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { label: 'Annual', used: emp.annual[0], total: emp.annual[1], color: 'bg-emerald-500' },
                ...
                <div key={b.label} className="bg-slate-50 rounded-xl p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-slate-600">{b.label}</span>
                    <span className="text-xs font-bold text-slate-900">{b.used}<span className="text-slate-400 font-normal">/{b.total}</span></span>
                  </div>
                  <ProgressBar value={b.used} max={b.total} color={b.color} />
                  <p className="text-[10px] text-slate-400 mt-1.5">{b.total - b.used} remaining</p>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm">
        ...
        <div className="divide-y divide-slate-50">
          {HOLIDAYS.map(h => (
            <div key={h.date + h.name} className="group flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50/60 transition-colors">
              {/* Date block */}
              <div className="w-12 flex-shrink-0 text-center">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider leading-none">{h.month}</p>
                <p className="text-xl font-black text-slate-900 leading-tight mt-0.5">{h.date.split(' ')[0]}</p>
              </div>
              ...
              {/* Name */}
              <span className="flex-1 text-sm font-semibold text-slate-800 truncate">{h.name}</span>

              {/* Badge */}
              <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border flex-shrink-0 ${h.color}`}>
                {h.type}
              </span>

              {/* Edit (hover) */}
              <button className="opacity-0 group-hover:opacity-100 text-[11px] font-medium text-slate-400 hover:text-slate-600 transition-all hidden sm:flex items-center gap-0.5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5 font-medium">
              Requests, balances, types and holidays
            </p>
          </div>
        </div>
