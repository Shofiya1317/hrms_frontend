  ...
  return (
    <DropdownMenu modal={false} {...openState} {...props}>
      <DropdownMenuTrigger asChild>
        <ToolbarButton
          className="lg:min-w-[130px] z-1000 hover:text-amber-600"
          pressed={openState.open}
          tooltip="Turn into"
          isDropdown
        >
          <SelectedItemIcon className="size-5 lg:hidden" />
          <span className="max-lg:hidden z-1000">{selectedItemLabel}</span>
        </ToolbarButton>
      </DropdownMenuTrigger>

      <DropdownMenuContent
        className="ignore-click-outside/toolbar min-w-0 z-[99999] bg-white opacity-100"
        align="start"
      ...
      >
        ...
        <DropdownMenuRadioGroup
          className="flex flex-col gap-0.5 z-1000"
          value={value}
        ...
        >
          {items.map(({ icon: Icon, label, value: itemValue }) => (
            <DropdownMenuRadioItem
              key={itemValue}
              className="min-w-[180px] z-1000"
              value={itemValue}
            >
              <Icon className="mr-2 size-5 z-1000" />
              {label}
            </DropdownMenuRadioItem>
          ))}
        </DropdownMenuRadioGroup>
